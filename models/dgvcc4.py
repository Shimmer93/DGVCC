...
class DensityRegressor(nn.Module):
    ...
    def forward(self, x):
        ...
        return y_shallow, y_inv, y_inv_new_, logits_inv, y
...
class DGVCCModel(nn.Module):
    ...
    def forward_joint_gen(self, x, z1, z2):
        ...
        loss_cyc = F.mse_loss(x_cyc, x)
        loss_div = -torch.clamp(F.mse_loss(x_gen, x_gen2), max=0.1)
        # loss_var = -torch.clamp(torch.mean(torch.var(res, dim=(2, 3))), max=0.1)
        loss_var = torch.mean(torch.mean(res**2, dim=(2, 3)))

        x_cat = torch.cat([x, x_gen])
        with torch.no_grad():
            y_shallow_cat, y_inv_cat, y_inv_new_cat, logits_inv_cat, d_cat = self.reg(x_cat)
        y_shallow = y_shallow_cat[:b]
        y_shallow_gen = y_shallow_cat[b:]
        logits_inv_gen = logits_inv_cat[b:]
        d_gen = d_cat[b:]

        loss_dissim = self._dissim_loss(y_shallow, y_shallow_gen)
        loss_novel = torch.mean(logits_inv_gen)

        return d_gen, loss_cyc, loss_div, loss_var, loss_dissim, loss_novel
    
    def forward_joint_reg(self, x, z1, z2):
        ...
        y_shallow_cat, y_inv_cat, y_inv_new_cat, logits_inv_cat, d_cat = self.reg(x_cat)
        y_inv = y_inv_cat[:b]
        y_inv_gen = y_inv_cat[b:]
        y_inv_new = y_inv_new_cat[:b]
        y_inv_new_gen = y_inv_new_cat[b:]

        loss_sim = F.mse_loss(y_inv, y_inv_gen) + F.mse_loss(y_inv_new, y_inv_new_gen)
        loss_rec = self._rec_loss(y_inv, y_inv_new) + self._rec_loss(y_inv_gen, y_inv_new_gen)

        return d_cat, loss_sim, loss_rec
