...
from models.dgvcc2 import DGVCCModel, ModelComponent
...
class DGVCCTrainer():
    ...
    def train_step(self, batch):
        ...
        if self.mode == DGMode.GENERATOR:
            ...
            d_cat, loss_cyc, loss_div, loss_dissim, loss_sim, loss_rec, loss_ortho = self.model.forward_joint(imgs, z1, z2)
            loss_den_cat = self.compute_count_loss_with_aug(d_cat, gt_datas, num_aug_samples=2)
            # loss = loss_den_cat + 10 * loss_cyc + 10 * loss_div + 100 * loss_dissim + 100 * loss_sim
            # loss = loss_den_cat + loss_sim + 10 * loss_cyc + loss_div + 100 * loss_dissim + 0.1 * loss_rec
            # loss = loss_den_cat + loss_sim + 10 * loss_cyc + loss_div + 1000 * loss_dissim + 0.1 * loss_rec
            loss = loss_den_cat + loss_sim + 10 * loss_cyc + 1000 * loss_dissim + 0.1 * loss_rec

            print(f'loss_den_cat: {loss_den_cat.item():.4f}, loss_cyc: {loss_cyc.item():.4f}, \
                    loss_div: {loss_div.item():.4f}, loss_dissim: {loss_dissim.item():.4f}, \
                    loss_rec: {loss_rec.item():.4f}')
